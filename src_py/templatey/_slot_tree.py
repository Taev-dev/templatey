...
def _resolve_pending_slot_tree(  # noqa: PLR0913
        pending_tree: PendingSlotTreeContainer,
        ...
        resolved_slot_tree_lookup: dict[TemplateClass, ConcreteSlotTreeNode],
    ...
    if slot_info is None:
        ...
        resolved_slot_item = (resolved_cls, SlotTreeNode())
        ...
        resolved_slot_item = (
            resolved_cls,
            SlotTreeNode([SlotTreeRoute.new(
                slot_name, slot_cls, SlotTreeNode())]))
        ...
        offset_pending_tree_root_node = PendingSlotTreeNode([
            SlotTreeRoute.new(
                slot_name,
                slot_cls,
    ...
    for nested_template_cls, nested_root_node in all_nested_slot_items:
        ...
        if dest_tree is None:
            dest_tree = enclosing_slot_tree_lookup[nested_template_cls] = (
                ...
                SlotTreeNode(id_=tree_after_insertions.id_))
    ...
    for (
        ...
        if dest_pending is None:
            # Copying over the ID isn't necessary, but it makes the
            # lineage a bit clearer if you need to debug things
            dest_tree =  PendingSlotTreeNode(id_=tree_after_insertions.id_)
            enclosing_pending_ref_lookup[forward_ref_key] = (
                PendingSlotTreeContainer(forward_ref_key, dest_tree))
        else:
...
def _apply_insertions(
        resolved_cls: TemplateClass,
        pending_tree: PendingSlotTreeNode,
        nested_template_type: TemplateClass,
        nested_root_node: ConcreteSlotTreeNode
        ) -> ConcreteSlotTreeNode: ...
@overload
def _apply_insertions(
        resolved_cls: TemplateClass,
        pending_tree: PendingSlotTreeNode,
        nested_template_type: ForwardRefLookupKey,
        nested_root_node: PendingSlotTreeNode
        ) -> PendingSlotTreeNode: ...
def _apply_insertions[T: ConcreteSlotTreeNode | PendingSlotTreeNode](
        resolved_cls: TemplateClass,
        pending_tree: PendingSlotTreeNode,
